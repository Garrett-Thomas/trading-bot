...
class StockPicker():
    ...
    def chooseBest(self):
        ...
        # if len(self.keys) < self.bDepth + self.sDepth:
        #     print("Buy and Sell depth are greater than the number of stocks!")
        #     quit()
        """
        ...
        for heldStock in reversed(toSell):
            for stock in currHoldings:
                if(heldStock == stock and len(confirmedSellStocks) < self.sDepth):
                    ...
                    print('Stock that I want to sell is {}'.format(heldStock))
        ...
        # Added the if because I don't want to buy stocks withought selling any first.
        if len(confirmedSellStocks) > 0:
            for stock in toBuy:

                currPicks['Buy'][stock] = stocksWithIndcData[self.keys[len(self.keys)-1]][stock]

        temp = dict()
        temp[datetime.now().strftime("%m/%d/%Y %H:%M:%S")] = currPicks

        currPicks = temp

        print(json.dumps(currPicks, indent=4))
        if(path.exists(parentDir + picksDir)):
            with open(parentDir + picksDir, "r+") as file:

                picksFile = json.load(file)
                picksFile.update(currPicks)
